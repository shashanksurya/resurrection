  ...
  initState(&state);         /* Initialize the state of the pipeline */
	...
	/* Modify newState stage-by-stage below to reflect the state of the pipeline after the cycle has executed */
        ...
        /* --------------------- ID stage --------------------- */
            ...
            if(get_immed(state.IDEX.instr) & (1<<15))
                {
                    offset = get_immed(state.IDEX.instr)-(1<<16);
                }
            newState.IDEX.branchTarget = state.IDEX.PCPlus4 + offset;
        /* --------------------- EX stage --------------------- */
            ...
            switch(get_opcode(state.IDEX.instr))
            {
                ...
                case LW:
                newState.EXMEM.aluResult = state.IDEX.readData1 + offset;
                break;
